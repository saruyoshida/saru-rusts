...
pub fn filter_supply() -> [Filters; FLC] { 
  [cv_filter(),
   cv_filter_width_var2(),
   cv_filter_width_var50(),
   ca_filter(),
  ]
}
// 定常速度フィルタそのまま
...
// 定常速度フィルタのQのvarを2.0に変更
fn cv_filter_width_var2() -> Filters {
  let mut kf = mv_filter1();
  // ノイズ設定
    // ノイズブロック作成
  let bn: SMatrix<f32, 2, 2> =  
    DiscreteWhiteNoise::noise_block(0.1, 2.0); 
    // プロセスノイズ設定
  kf.fb.Q.copy_from(&bn);
  Filters::Kf1(kf)
}
// 定常速度フィルタのQのvarを50に変更
fn cv_filter_width_var50() -> Filters {
  let mut kf = mv_filter1();
  // ノイズ設定
    // ノイズブロック作成
  let bn: SMatrix<f32, 2, 2> =  
    DiscreteWhiteNoise::noise_block(0.1, 50.); 
    // プロセスノイズ設定
  kf.fb.Q.copy_from(&bn);
  Filters::Kf1(kf)
}
