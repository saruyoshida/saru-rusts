...
fn main() -> ! {
    ...
    let mut wio_clock = WioClock::new(&mut display);
    ...
    let mut wio_button = WioButtons::build(
        (0i32, 0i32, 23i32, 1i32, String::from("hour")),
        (0i32, 0i32, 59i32, 1i32, String::from("minute")),
        (0i32, 0i32, 59i32, 1i32, String::from("second"))
    );
...
//        delay.delay_ms(1000 as u16);
        ...
        wio_clock.update(time.hours, time.minutes, time.seconds);
        ...
        if let Some(press) = consumer.dequeue() {
            ...
            if let Some(now_time) =
              wio_button.button_pulled(press.button) 
            {
