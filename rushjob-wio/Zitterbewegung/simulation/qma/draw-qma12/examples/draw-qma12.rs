...
fn main() -> ! {
  ...
  let dxyz = |xt:T, yt:T, zt:T, rt:T| (
    -(1./(2.*rt)+2./(2.*zt*zt-xt*xt-yt*yt))
    *xt, // DX(t)
    -(1./(2.*rt)+2./(2.*zt*zt-xt*xt-yt*yt))
    *yt, // DY(t)
    -(1./(2.*rt)-4./(2.*zt*zt-xt*xt-yt*yt))
    *zt, // DZ(t)
  );
  // 見本経路描画                         
  let xyz0 = (3., 4., 5.); // 開始位置(x,y,z)
  let dt   = 0.01;         // Δt
  let cnt  = 300000;       // 計算回数
  let step = 20;           // 描画間引き数
  ...
  .for_each(|(i, (dx, dy, dz, _dr))| {
    ...
    // スライス表示
    let (sx, _sy, sz) = sv.view(dx, dy, dz);
    graph_obj[3].set_data(sx, sz);
    // 描画
