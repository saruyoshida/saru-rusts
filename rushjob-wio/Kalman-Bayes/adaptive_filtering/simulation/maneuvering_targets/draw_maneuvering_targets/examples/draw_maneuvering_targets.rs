...
// フィルタ供給
use filter_supply_maneuvering_targets::*; 
// グラフ供給
use graph_supply_maneuvering_targets::*;  
// シミュレーションデータ
...
fn main() -> ! {
  ...
  // 繰り返し -----------------------------
  let dt = 0.1;
  let gd = GenerateData::new(150, 2.);
  for (i, (pos, z)) in gd.into_iter()
                         .enumerate() {
    ...
    { 
      ...
      f.update();       // 更新
      // グラフ描画
      g[0].set_data(i as f32 * dt, pos.0); 
